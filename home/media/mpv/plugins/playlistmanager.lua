...
-- { "%s*[%[%(].-[%]%)]%s*": "" },
  ...
  playlist_savepath = "~/sync/databases/mpv_playlists",
...
function save_playlist(filename)
  ...
  if utils.readdir(savepath) == nil then
    ...
    local res = utils.subprocess({ args = unix_args, cancellable = false })
  ...
  local name = filename or "last_playlist.m3u"
